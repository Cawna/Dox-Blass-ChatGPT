...
function title() {
	...
	console.log(chalk.yellow(`\n              ${chalk.yellow('[ Creado por TIAGO ]')}\n\n${chalk.red('Bot OpenAI')} : ${chalk.white('WhatsApp Bot OpenAI')}\n${chalk.red('Contactame por WhatsApp')} : ${chalk.white('+51 946 352 266')}\n`))
}
...
const connectToWhatsApp = async () => {
	...
	conn.ev.on('connection.update', (update) => {
          ...
          const { connection, lastDisconnect } = update
            ...
            if (connection == 'open') {
                console.log(chalk.yellow('💡Activo y funcionando correctamente\n'))
            }        
            })
...
if (anu.action == 'add') {
let captionwel = '@subject\n\n@user Bienvenido (a)\n\nDisfruta del grupo, no olvides leer la descripción para estar informado de la temática del grupo.'
